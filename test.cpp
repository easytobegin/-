...
{  
	...
	//CompareString(m_szDirectory, -1, lpszDirectory, -1, TRUE)
	//else if(strcmp(m_szDirectory,lpszDirectory) != 0){
	//	cout<<"文件夹没有改变"<<endl;
	//	TRACE("Not Change Directory.\n");  
	//	return FALSE;  
	//}
	else if(CSTR_EQUAL != CompareString(Locale, NORM_IGNORECASE, m_szDirectory, _tcslen(m_szDirectory), lpszDirectory, _tcslen(lpszDirectory)) - 2)  
	{  
		TRACE("Not Change Directory.\n");  
		return FALSE;  
	}
